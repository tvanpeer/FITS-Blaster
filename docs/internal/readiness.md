...
**Model**: Freemium (free core, paid unlock via In-App Purchase) **Languages**: English only at launch **Status**: Work in progress — items marked \[ \] are open, \[x\] are done.
...
- [x] Decide on support e-mail address — will create once domain is registered (section 8)
...
- [x] Rename scheme from "Claude FITS Viewer" to "FITS Blaster" in Xcode (cosmetic, low priority)
